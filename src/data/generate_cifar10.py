...
def create_cifar10_subset(dataset, num_examples_by_class=10):
    """
    Crée un sous-dataset CIFAR10 avec un nombre spécifique d'exemples par classe
    ...
    print(f"Sous-dataset CIFAR10 créé avec {len(subset_dataset)} exemples")
    print(f"Sauvegardé dans 'cifar10_subset_{num_examples_by_class}_per_class.pth'")

    return subset_dataset


# Fonction pour charger le sous-dataset
def load_cifar10_subset(num_examples_by_class=10):
    """
    Charge un sous-dataset CIFAR10 précédemment créé
    """
    data = torch.load(
        f"data/datasets/cifar10_subset_{num_examples_by_class}_per_class.pth"
    )
    return data["dataset"]
...
if __name__ == "__main__":
    ...
    print("Téléchargement de CIFAR10...")
    cifar10_dataset_TRAIN = datasets.CIFAR10(
        "data/datasets", train=True, download=True, transform=transform
    )
    cifar10_dataset_TEST = datasets.CIFAR10(
        "data/datasets", train=False, download=True, transform=transform
    )

    print("CIFAR10 téléchargé avec succès !")

    print("\nCréation du sous-dataset CIFAR10...")
    subset = create_cifar10_subset(cifar10_dataset_TRAIN, num_examples_by_class=10)
